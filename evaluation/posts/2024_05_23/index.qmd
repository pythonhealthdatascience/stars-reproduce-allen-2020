...
* Consider what to do when there are multiple versions of the code available (such as in this example, where we had some minor differences between GitHub and Zenodo), and whether there are any recommendations I can make on this in protocol (likely along the lines of using that which fits timing-wise with article publication).
* Add recommendations on what to do when they don't include any control of randomness with seeds etc.
* When making comparison of results of figure by eye, part of this decision on how similar they are will be subjective or based slightly on whether interpretations still hold - is that correct? We don't want to focus too much on that - but we anticipate we will often not get exactly similar results due to randomness. Look over my suggestions on decision making for figure similarity and decide whether they fit our situation.

...
Compared my decisions for the best practice audit against those made in Monks and Harper. Don't think I would normally plan to do this. Their GitHub is [TomMonks/des_sharing_lit_review](https://github.com/TomMonks/des_sharing_lit_review), which provides the file `share_sim_data_extract.zip`. I used the provided code to clean this and saved it as `share_sim_data_extract_clean.csv`, which can then view here:

```{python}
import pandas as pd

# Import review results
review = pd.read_csv('share_sim_data_extract_clean.csv')
...
* Review doesn't mention Orcid
* Review doesn't mention license
* Review doesn't mention README
* Review doesn't mention steps to run code
* Review doesn't mention formal or informal dependency management
* Review doesn't mention code testing
* Review doesn't mention local execution
...
* Entry/exit points - they are in model on days they need dialysis, and then leave again? unless get unallocated? and then they never return if their state turns to dead? this isn't really spelled out though.
...
* Suggest to detail those uncertainties within logbook
* Any advice can give following uncertainties raised above
